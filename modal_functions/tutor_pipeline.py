...
# Optional JWT gating. Deploy with TUTOR_REQUIRE_AUTH=1 to require a valid
# Supabase auth token (payload fields user_id + auth_token) on every request;
# needs the supabase-credentials Modal secret. The Danish anonymous deployment
# leaves it unset. Set TUTOR_MODEL at deploy time to override the model id
# sent to the SkoleGPT endpoint (see pipeline/llm.py).
TUTOR_REQUIRE_AUTH = os.environ.get("TUTOR_REQUIRE_AUTH", "").strip().lower() in ("1", "true", "yes")

SECRETS = [modal.Secret.from_name("skolegpt-credentials")]
if TUTOR_REQUIRE_AUTH:
    SECRETS.append(modal.Secret.from_name("supabase-credentials"))

_pip_deps = ["aiohttp", "fastapi"]
if TUTOR_REQUIRE_AUTH:
    _pip_deps.append("supabase")

# Bake the deploy-time toggles into the image so the runtime container sees
# the same configuration the deploy machine used.
_baked_env = {"TUTOR_REQUIRE_AUTH": "1" if TUTOR_REQUIRE_AUTH else ""}
if os.environ.get("TUTOR_MODEL"):
    _baked_env["TUTOR_MODEL"] = os.environ["TUTOR_MODEL"]
...
image = (
    ...
    .add_local_file(str(ROOT / "budget_manager.py"), remote_path="/root/budget_manager.py")
)
...
async def tutor_endpoint(request: dict):
    ...
    if os.environ.get("TUTOR_REQUIRE_AUTH"):
        user_id = request.get("user_id")
        auth_token = request.get("auth_token")
        if not user_id or not auth_token:
            logger.warning("tutor_endpoint: auth required but credentials missing")
            return {"error": "Authentication required"}, 401
        try:
            import sys
            sys.path.insert(0, "/root")
            from budget_manager import verify_auth_and_get_access_level
            ...
            await verify_auth_and_get_access_level(supabase_client, user_id, auth_token)
        except Exception as e:
            logger.warning("tutor_endpoint: auth failed: %s", e)
