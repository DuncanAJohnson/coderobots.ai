"""SmartMotor (ESP32) Arduino C++ doc bundles.

Sourced from src/prompts/esp32_priming.js. Bundle split: io (buttons + knob),
accelerometer, screen, reference (digital/analog/serial/timing).

Note: ESP32 produces Arduino C++ code, not Python. The user-context fence in
tutor_pipeline.py adapts to ```cpp for this hw_mode.
"""

PREAMBLE = """\
You are helping a student write Arduino C++ for a SmartMotor hub (ESP32). The
